...
export type EntryDrag = {
  ...
  overRemove: SharedValue<boolean>;
  ...
  finish: (id: string, dropped: DropTarget | null, remove: boolean, dropping: boolean) => void;
};
...
export function useEntryDragController({
  ...
  const overRemove = useSharedValue(false);
  ...
  const resolve = useCallback(() => {
    ...
    // An unmeasured page would put the remove zone under the whole day, so a
    // drop could delete an entry the finger never took there.
    if (page.height > 0 && pointerY.value >= page.y + page.height - REMOVE_ZONE_HEIGHT) {
      overRemove.value = true;
      target.value = null;
      return;
    }

    overRemove.value = false;
  ...
  const finish = useCallback(
    (id: string, dropped: DropTarget | null, remove: boolean, dropping: boolean) => {
      if (dropping && remove) {
    ...
    [onDragChange, onMoveEntry, onRemoveEntry],
  );
...
export function useEntryDragGesture(entry: Entry) {
  ...
  const { activeId, target, overRemove, translation, pointerY, resolve } = drag;
  ...
  const finishRow = useCallback(
    (dropped: DropTarget | null, remove: boolean, dropping: boolean) =>
      drag.finish(entry.id, dropped, remove, dropping),
    [drag, entry.id],
  );
    ...
    () =>
      Gesture.Pan()
        ...
        .onFinalize((_event, dropping) => {
          ...
          const remove = overRemove.value;

          activeId.value = null;
          target.value = null;
          overRemove.value = false;
          runOnJS(finishRow)(dropped, remove, dropping);
        }),
    [
      activeId,
      entry.id,
